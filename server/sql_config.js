...
module.exports = {
  ...
  /*Show message that the data was received*/
	...
	selectFolders: function(userId){
		...
		con.query('SELECT f.folderId, f.folderName FROM Folder f JOIN  UserFolder uf ON f.folderId=uf.folderId WHERE  uf.userId=?;', userId, function(err,rows){
			if(err) throw err;
	...
	selectFiles: function(folderId){
		con.query('SELECT fileName, fileSize FROM File WHERE folderId = ?;', folderId, (err,rows) => {
      if(err) throw err;
      console.log('Data received from Db:\n');
      console.log(rows);
    });
    ...
    con.query('INSERT INTO User (username, userPass, email) VALUES(?,?,?);')
    /*Add a user.*/
    /*Show message that the data was received*/
	},
    ...
    con.query('INSERT INTO File (fileName, fileSize, folderId) VALUES(?,?,?);')
    /*Add a user.*/
    /*Show message that the data was received*/
  }
}
